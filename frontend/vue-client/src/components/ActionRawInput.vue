...
import { actionRawService } from '../services/actionRaw';
...
const submitTranscription = async () => {
  ...
  try {
    await actionRawService.createActionRaw(transcription.value.trim());
    successMessage.value = 'Raw action saved successfully!';
    transcription.value = '';
    interimText.value = '';
    ...
    console.error('Error saving raw action:', error);
    errorMessage.value = error.response?.data?.detail || error.message || 'Failed to save raw action. Please try again.';
  } finally {
